...
class CropBox():
    def __init__(self,config,alphapred):
        ...
        self._box_min_size_heigth = config.BBOX.MIN_SIZE_HEIGTH
        self._box_min_size_width = config.BBOX.MIN_SIZE_WIDTH
            ...
            for image_alphalabel in self._alphapred:
                if img_name == image_alphalabel[0]._image_id:
                    ...
                    for label in image_alphalabel:
                        ...
                        if (int((new_img.shape[0]*100)/heigth_org) > self._box_min_size_heigth and int((new_img.shape[1]*100)/width_org) > self._box_min_size_width):# condition taille de la box
                            ...
                            # if confidence_keypoints[0] > self._tresh and confidence_keypoints[-1] > self._tresh # condition sur la condidence : cas sur la 1er et last value
                                ...
                                if area[-1]==max(area) and self._only_one == 'yes': #case where only_one = yes, we save only the prediction with biggest area
                                    ...
                                    i +=1
                                elif i == 0 and self._only_one!= 'yes': #case where only_one = no, we save the first prediction
                                    image.binary = new_img
                                    image._name = new_img_name
                                    new_folder = os.path.join(os.getcwd(),'datasets',self._datasetname, 'bbox/' + self._inputfolder)
                                    image._path= os.path.join(new_folder , new_img_name)
                                    i +=1
                                elif i != 0 and self._only_one!= 'yes': #case where only_one = no, we save the other predictioni
